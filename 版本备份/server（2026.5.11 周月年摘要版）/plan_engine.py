...
def _extract_relationship_shadow_hint(text: Any) -> str:
    sample = str(text or "").strip()
    if not sample or not _looks_like_user_contact(sample):
        return ""
    sample = re.sub(r"\s+", " ", sample).strip()
    sample = re.sub(r"[“”\"']", "", sample)
    sample = re.sub(r"(向用户发消息|给用户发消息|联系用户|同步请求|问候|终端陪伴)", "", sample)
    sample = sample.strip("，。；;、 ")
    return sample[:120]
...
def _normalize_plan_structure_fields_v2(raw: dict[str, Any], *, fallback_activity: str) -> dict[str, Any]:
    ...
    shadow_candidates = [
        _extract_relationship_shadow_hint(raw_activity_text),
        _extract_relationship_shadow_hint(raw_objective_text),
    ]
    if not any(outline.values()):
        ...
        for value in outline.values():
            hint = _extract_relationship_shadow_hint(value)
            if hint:
                shadow_candidates.append(hint)
    ...
    relationship_shadow_hint = ""
    for candidate in shadow_candidates:
        if candidate:
            relationship_shadow_hint = candidate
            break
    payload.update(
        {
            "dominant_mode": dominant_mode or "internal_maintenance",
            ...
            "relationship_shadow_hint": relationship_shadow_hint,
            "relationship_shadow_detected": bool(relationship_shadow_hint),
        }
    )
    return payload
