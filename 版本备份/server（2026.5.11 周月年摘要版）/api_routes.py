...
from server.memory_summary import MemorySummaryEngine
...
def set_dependencies(
    ...
    plan_engine=None,
    npc_engine=None,
    memory_summary_engine=None,
):
    global _db, _state_machine, _memory_store, _prompt_manager, _evolution_engine
    global _llm_client, _env_llm_client, _snapshot_llm_client, _plan_engine, _npc_engine, _memory_summary_engine
    _db = db
    ...
    _memory_summary_engine = memory_summary_engine
...
@router.get("/memory-summaries/{summary_level}/status")
async def memory_summary_status(summary_level: str):
    if _memory_summary_engine is None:
        raise HTTPException(500, "Memory summary engine is not initialized")
    return await _memory_summary_engine.get_status(summary_level)


@router.get("/memory-summaries/{summary_level}/runs")
async def memory_summary_runs(summary_level: str, status: str | None = None, limit: int = 20):
    if _memory_summary_engine is None:
        raise HTTPException(500, "Memory summary engine is not initialized")
    return await _memory_summary_engine.list_runs(summary_level, status=status, limit=limit)


@router.get("/memory-summaries/{summary_level}/pending-preview")
async def memory_summary_pending_preview(summary_level: str):
    if _memory_summary_engine is None:
        raise HTTPException(500, "Memory summary engine is not initialized")
    data = await _memory_summary_engine.get_pending_preview(summary_level)
    if not data:
        raise HTTPException(404, "No pending memory summary preview")
    return data


@router.post("/memory-summaries/preview")
async def memory_summary_preview(req: MemorySummaryPreviewRequest):
    if _memory_summary_engine is None:
        raise HTTPException(500, "Memory summary engine is not initialized")
    try:
        return await _memory_summary_engine.preview(
            summary_level=req.summary_level,
            window_start=req.window_start,
            window_end=req.window_end,
            store_pending=req.store_pending,
            source="manual",
        )
...
@router.put("/memory-summaries/{summary_level}/pending-preview")
async def memory_summary_update_pending(summary_level: str, req: EvolutionApplyRequest):
    if _memory_summary_engine is None:
        raise HTTPException(500, "Memory summary engine is not initialized")
    return await _memory_summary_engine.save_pending_preview(summary_level, req.preview, source="manual_edit")


@router.post("/memory-summaries/{summary_level}/apply")
async def memory_summary_apply(summary_level: str, req: EvolutionApplyRequest):
    if _memory_summary_engine is None:
        raise HTTPException(500, "Memory summary engine is not initialized")
    return await _memory_summary_engine.apply(summary_level, req.preview)
