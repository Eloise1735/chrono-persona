...
CREATE TABLE IF NOT EXISTS life_digest_nodes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    node_date TEXT NOT NULL,
    source_kind TEXT NOT NULL DEFAULT 'merged',
    title TEXT NOT NULL DEFAULT '',
    summary TEXT NOT NULL DEFAULT '',
    life_domain TEXT NOT NULL DEFAULT 'routine',
    salience REAL NOT NULL DEFAULT 0.5,
    novelty_score REAL NOT NULL DEFAULT 0.5,
    carry_forward REAL NOT NULL DEFAULT 0.5,
    linked_snapshot_id INTEGER,
    linked_trace_id INTEGER,
    linked_env_ref TEXT,
...
CREATE TABLE IF NOT EXISTS memory_summary_runs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    source_summary_run_ids_json TEXT NOT NULL DEFAULT '[]',
    source_life_node_ids_json TEXT NOT NULL DEFAULT '[]',
    summary_pack_json TEXT NOT NULL DEFAULT '[]',
    bridge_pack_json TEXT NOT NULL DEFAULT '[]',
    event_line_digest_json TEXT NOT NULL DEFAULT '[]',
    day_digest_json TEXT NOT NULL DEFAULT '{}',
    editor_notes TEXT
);
"""
...
class Database:
    ...
    async def _ensure_schema_updates(self):
        ...
        await self._ensure_column("life_digest_nodes", "linked_env_ref", "TEXT")
        await self._ensure_column("life_digest_nodes", "source_fingerprint", "TEXT NOT NULL DEFAULT ''")
        ...
        await self._ensure_column("memory_summary_runs", "source_life_node_ids_json", "TEXT NOT NULL DEFAULT '[]'")
        await self._ensure_column("memory_summary_runs", "day_digest_json", "TEXT NOT NULL DEFAULT '{}'")
    ...
    async def insert_life_digest_node(self, node: LifeDigestNode) -> int:
        cursor = await self.conn.execute(
            """INSERT INTO life_digest_nodes
               (node_date, source_kind, title, summary, life_domain, salience, novelty_score, carry_forward,
                linked_snapshot_id, linked_trace_id, linked_env_ref, source_fingerprint, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                node.node_date,
                node.source_kind,
                ...
                node.novelty_score,
                node.carry_forward,
                node.linked_snapshot_id,
                node.linked_trace_id,
                node.linked_env_ref,
    ...
    async def list_life_digest_nodes(
        self,
        *,
        start_date: str | None = None,
        end_date: str | None = None,
        node_date: str | None = None,
        limit: int = 24,
    ) -> list[LifeDigestNode]:
        sql = "SELECT * FROM life_digest_nodes WHERE 1=1"
        params: list = []
        if node_date:
            sql += " AND node_date = ?"
            params.append(node_date)
        if start_date:
            sql += " AND node_date >= ?"
            params.append(start_date)
        if end_date:
            sql += " AND node_date <= ?"
            params.append(end_date)
        sql += " ORDER BY node_date DESC, salience DESC, updated_at DESC, id DESC LIMIT ?"
        params.append(limit)
        async with self.conn.execute(sql, params) as cur:
            rows = await cur.fetchall()
            return [LifeDigestNode(**dict(r)) for r in rows]

    async def get_life_digest_node_by_fingerprint(
        self,
        *,
        node_date: str,
        ...
        async with self.conn.execute(
            """SELECT * FROM life_digest_nodes
               WHERE node_date = ? AND source_fingerprint = ?
               ORDER BY updated_at DESC, id DESC LIMIT 1""",
            (node_date, source_fingerprint),
        ) as cur:
            row = await cur.fetchone()
            return LifeDigestNode(**dict(row)) if row else None

    async def update_life_digest_node(self, node_id: int, **fields) -> None:
        if not fields:
            return
        fields["updated_at"] = format_utc_instant_z(datetime.utcnow())
        set_clause = ", ".join(f"{key} = ?" for key in fields)
        ...
        await self.conn.execute(
            f"UPDATE life_digest_nodes SET {set_clause} WHERE id = ?",
            values,
        )
        await self.conn.commit()
    ...
    async def insert_memory_summary_run(self, run: MemorySummaryRun) -> int:
        cursor = await self.conn.execute(
            """INSERT INTO memory_summary_runs
               (summary_level, window_start, window_end, created_at, applied_at, status,
                source_event_ids_json, source_summary_run_ids_json, source_life_node_ids_json,
                summary_pack_json, bridge_pack_json, event_line_digest_json, day_digest_json, editor_notes)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                run.summary_level,
                run.window_start,
                ...
                run.applied_at,
                run.status,
                run.source_event_ids_json,
                run.source_summary_run_ids_json,
                run.source_life_node_ids_json,
                run.summary_pack_json,
                run.bridge_pack_json,
                run.event_line_digest_json,
                run.day_digest_json,
                run.editor_notes,
            ),
        )
    ...
    async def get_latest_memory_summary_run(
        self,
        *,
        summary_level: str,
        status: str | None = None,
    ) -> MemorySummaryRun | None:
        sql = "SELECT * FROM memory_summary_runs WHERE summary_level = ?"
        params: list = [summary_level]
        if status:
            sql += " AND status = ?"
            params.append(status)
        sql += " ORDER BY window_end DESC, created_at DESC, id DESC LIMIT 1"
        async with self.conn.execute(sql, params) as cur:
            row = await cur.fetchone()
            return MemorySummaryRun(**dict(row)) if row else None

    async def list_memory_summary_runs(
        self,
        *,
        summary_level: str,
        status: str | None = None,
        limit: int = 20,
    ) -> list[MemorySummaryRun]:
        sql = "SELECT * FROM memory_summary_runs WHERE summary_level = ?"
        params: list = [summary_level]
        if status:
            sql += " AND status = ?"
            params.append(status)
        sql += " ORDER BY window_end DESC, created_at DESC, id DESC LIMIT ?"
        params.append(limit)
        async with self.conn.execute(sql, params) as cur:
            rows = await cur.fetchall()
            return [MemorySummaryRun(**dict(r)) for r in rows]

    async def get_memory_summary_runs_in_window(
        self,
        *,
        summary_level: str,
        window_start: str,
        ...
        sql = (
            "SELECT * FROM memory_summary_runs "
            "WHERE summary_level = ? AND window_start >= ? AND window_end <= ?"
        )
        params: list = [summary_level, window_start, window_end]
        if status:
            sql += " AND status = ?"
            params.append(status)
        sql += " ORDER BY window_start ASC, created_at DESC, id DESC LIMIT ?"
        params.append(limit)
        async with self.conn.execute(sql, params) as cur:
            rows = await cur.fetchall()
            return [MemorySummaryRun(**dict(r)) for r in rows]

    async def supersede_memory_summary_runs(self, *, summary_level: str, status: str) -> None:
        await self.conn.execute(
            """UPDATE memory_summary_runs
               SET status = 'superseded'
               WHERE summary_level = ? AND status = ?""",
            (summary_level, status),
        )
        await self.conn.commit()
