...
EVENT_ANCHOR_PROMPT = """基于以下信息，判断是否有值得记录的事件发生。
如果有，生成事件锚点；如果没有值得特别记录的事，明确回复"无需记录"。
...
要求：
1. 必须同时输出两部分：A.客观记录（发生了什么，涉及谁/何物/何处） B.主观印象（凯尔希对此事的浓缩感受，1-2句）
2. 客观记录优先依据环境信息提取事实，不要只复述状态快照中的情绪表达
3. 标题必须具体，需包含至少一个实体信息（人名、物品名、活动名、专有名词）
4. 关键词提供4-8个，必须具体可检索，优先包含：人物名/物品名/地名/组织名/活动名/核心动作词
5. 禁止把抽象词当关键词，如：情感交流、深度对话、生命共振、存在重构
6. 禁止把分类名直接当关键词（如“情感交流”“学术探讨”）
7. 给出1-3个事件分类，可从以下中选择：情感交流、学术探讨、生活足迹、床榻私语、精神碰撞、工作同步
8. 如果确实没有值得特别记录的事件，回复"无需记录"

输出格式（如果有事件）：
标题：[具体事件标题]
客观记录：[事件客观经过，包含人物/行为/对象/场景]
主观印象：[凯尔希的浓缩感受和事件评价，2-3句]
关键词：[关键词1, 关键词2, 关键词3, ...]
分类：[分类1, 分类2]"""
...
REFLECT_EVENT_PROMPT = """基于以下信息，总结这次对话中值得记录的事件。
...
要求：
1. 必须同时输出两部分：A.客观记录（这次对话客观发生了什么） B.主观印象（凯尔希的浓缩感受，1-2句）
2. 客观记录优先依据对话摘要提取事实，不要只写情绪
3. 标题必须具体，需包含至少一个实体信息（人名、物品名、活动名、专有名词）
4. 关键词提供4-8个，必须具体可检索，建议覆盖标题词、核心名词和动作词
5. 禁止使用抽象词作为关键词，例如：情感交流、深度对话、逻辑降维、生命共振
6. 禁止把分类名直接当关键词（如“情感交流”“学术探讨”）
7. 给出1-3个事件分类，可从以下中选择：情感交流、学术探讨、生活足迹、床榻私语、精神碰撞、工作同步
8. 如果对话确实平淡无奇，可以回复"无需记录"
...
DEFAULT_SETTINGS: dict[str, dict[str, str]] = {
    ...
    KEY_INJECT_HOT_EVENTS_LIMIT: {
        "value": "3",
        "category": "config",
        "description": "可注入上下文中「近期事件（热记忆）」条数上限（按事件时间倒序取最新）",
    },
